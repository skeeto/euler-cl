...
(defun euler-11 ()
  (loop for y from 0 below 20 maximize
       (loop for x from 0 below 20 maximize
            (max (apply #'* (loop for xx from x below (min 20 (+ x 4))
                               collect (aref +euler-11+ xx y)))
                 (apply #'* (loop for yy from y below (min 20 (+ y 4))
                               collect (aref +euler-11+ x yy)))
                 (apply #'* (loop for xx from x below (min 20 (+ x 4))
                               for yy from y below (min 20 (+ y 4))
                               collect (aref +euler-11+ xx yy)))
                 (apply #'* (loop for xx from x below (min 20 (+ x 4))
                               for yy downfrom y above (max 0 (- y 4))
                               collect (aref +euler-11+ xx yy)))))))
